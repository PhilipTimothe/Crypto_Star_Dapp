...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenName = await instance.getTokenName();
  let tokenSymbol = await instance.getTokenSymbol();
  let name = "Star";
  let symbol = "STR";
...
it("lets a user transfer a star", async () => {
  ...
  let user1 = accounts[1];
  let user2 = accounts[2];
  await instance.createStar("Star is Born!", tokenId, { from: user1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let lookUpInfo = await instance.lookUptokenIdToStarInfo(tokenId);
  // 3. Verify if you Star name is the same
  assert.equal(starName, lookUpInfo);
});
